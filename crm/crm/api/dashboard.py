...
# ──────────────────────────────────────────────────────────────────────
# EduFlow-specific dashboard widgets
# ──────────────────────────────────────────────────────────────────────

def get_messenger_bot_leads(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	"""Count leads generated by the Messenger Bot."""
	diff = frappe.utils.date_diff(to_date, from_date)
	if diff == 0:
		diff = 1
	...
	current_cond = (Lead.creation >= from_date) & (Lead.creation < to_date_plus_one) & (Lead.source == "Messenger Bot")
	if user:
		current_cond = current_cond & (Lead.lead_owner == user)

	prev_cond = (Lead.creation >= prev_from_date) & (Lead.creation < from_date) & (Lead.source == "Messenger Bot")
	if user:
		prev_cond = prev_cond & (Lead.lead_owner == user)
	...
	return {
		"title": _("Messenger Bot Leads"),
		"tooltip": _("Leads generated from Facebook Messenger bot"),
		"value": current,
		"delta": delta,
		"deltaSuffix": "%",
	}
...
def get_facebook_ads_leads(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	"""Count leads generated by Facebook Lead Ads."""
	diff = frappe.utils.date_diff(to_date, from_date)
	if diff == 0:
		diff = 1
	...
	current_cond = (Lead.creation >= from_date) & (Lead.creation < to_date_plus_one) & (Lead.source == "Facebook")
	if user:
		current_cond = current_cond & (Lead.lead_owner == user)

	prev_cond = (Lead.creation >= prev_from_date) & (Lead.creation < from_date) & (Lead.source == "Facebook")
	if user:
		prev_cond = prev_cond & (Lead.lead_owner == user)
	...
	return {
		"title": _("Facebook Ads Leads"),
		"tooltip": _("Leads generated from Facebook Lead Ads"),
		"value": current,
		"delta": delta,
		"deltaSuffix": "%",
	}
...
def get_leads_with_phone(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	"""Count leads that have a phone number."""
	diff = frappe.utils.date_diff(to_date, from_date)
	if diff == 0:
		diff = 1
	...
	phone_cond = (Lead.mobile_no.isnotnull()) & (Lead.mobile_no != "")

	current_cond = (Lead.creation >= from_date) & (Lead.creation < to_date_plus_one) & phone_cond
	if user:
		current_cond = current_cond & (Lead.lead_owner == user)

	prev_cond = (Lead.creation >= prev_from_date) & (Lead.creation < from_date) & phone_cond
	if user:
		prev_cond = prev_cond & (Lead.lead_owner == user)
	...
	return {
		"title": _("Leads with Phone"),
		"tooltip": _("Leads that provided a phone number"),
		"value": current,
		"delta": delta,
		"deltaSuffix": "%",
	}
...
def get_leads_by_course_interest(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	"""Donut chart of leads grouped by course interest."""
	Lead = DocType("CRM Lead")
	...
	query = (
		frappe.qb.from_(Lead)
		.select(
			Coalesce(IfNull(Lead.course_interest, ""), "Chưa xác định").as_("course"),
			Count("*").as_("count"),
		)
		.where(Date(Lead.creation).between(from_date, to_date))
		.groupby(Lead.course_interest)
		.orderby(Count("*"), order=frappe.qb.desc)
	)
	...
	for row in result:
		if not row["course"] or row["course"] == "":
			row["course"] = "Chưa xác định"

	return {
		"data": result or [],
		"title": _("Leads by Course Interest"),
		"subtitle": _("Distribution of leads by their course interest"),
		"categoryColumn": "course",
		"valueColumn": "count",
	}
...
def get_leads_by_branch(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	...
	for row in result:
		if not row["branch"] or row["branch"] == "":
			row["branch"] = "Chưa phân"

	return {
		"data": result or [],
		"title": _("Leads by Branch"),
		"subtitle": _("Distribution of leads across branches"),
		"xAxis": {
			"title": _("Branch"),
			"key": "branch",
			"type": "category",
		...
		"series": [
			{"name": "count", "type": "bar", "showDataPoints": True},
		],
	}


def get_lead_trend_by_source(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	"""Line chart showing daily leads split by source (Messenger Bot vs Facebook vs Other)."""
	Lead = DocType("CRM Lead")
	...
	messenger_cond = Lead.source == "Messenger Bot"
	facebook_cond = Lead.source == "Facebook"

	query = (
		frappe.qb.from_(Lead)
		.select(
			Date(Lead.creation).as_("date"),
			Count(Case().when(messenger_cond, Lead.name).else_(None)).as_("messenger"),
			Count(Case().when(facebook_cond, Lead.name).else_(None)).as_("facebook"),
			Count(Case().when(~messenger_cond & ~facebook_cond, Lead.name).else_(None)).as_("other"),
		)
		.where(Date(Lead.creation).between(from_date, to_date))
	...
	return {
		"data": data,
		"title": _("Lead Trend by Source"),
		"subtitle": _("Daily leads split by acquisition channel"),
		"xAxis": {
			"title": _("Date"),
			"key": "date",
			"type": "time",
		...
		"series": [
			{"name": "messenger", "type": "line", "showDataPoints": True},
			{"name": "facebook", "type": "line", "showDataPoints": True},
			{"name": "other", "type": "line", "showDataPoints": True},
		],
	}
