...
Scope:
- `docker/` (chatwoot override, n8n, caddy), `crm/docker/` compose override, `scripts/`.
...
- Never destroy the `chatwoot`, `crm`, or `n8n` projects' volumes; test destructive or from-scratch steps in a separate `-p <name>-verify` project.
